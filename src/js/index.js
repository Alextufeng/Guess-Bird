...
let qurLang = localStorage.getItem('qurLang') ? JSON.parse(localStorage.getItem('qurLang')) : 'en';
langChange.innerHTML = localStorage.getItem('langChange') ? JSON.parse(localStorage.getItem('langChange')) : `Ru`;
let birds = localStorage.getItem('lang') ? JSON.parse(localStorage.getItem('lang')) : birdsDataEn;
...
game.forEach(btn => btn.addEventListener('click', function() {
    toPlay();
}));
...
let audioOn = localStorage.getItem('sound') ? JSON.parse(localStorage.getItem('sound')) : true;
audioBtn.innerHTML = localStorage.getItem('soundBtn') ? JSON.parse(localStorage.getItem('soundBtn')) : `<img src="./assets/icons/volume_on.svg" alt="">`;
...
function rightBirdPush() {
    document.querySelector('.qur-bird__img').innerHTML = `<img src='${targetBird.image}'>`;
...
nextBtn.addEventListener('click', function() {
    ...
    document.querySelector('.what-bird-description').classList.add('hidden');
    document.querySelector('.qur-bird__img').innerHTML = `<img src="./assets/images/question__bird.jpg">`;
    document.querySelector('.qur-bird__name').innerHTML = `<h3>*******</h3>`;
    if (nextBtn.innerHTML == 'Go To Results!' || nextBtn.innerHTML == 'Перейти к результату!') {
        finish();
    }
    nextQuestion();
});
...
answers.forEach(item => item.addEventListener('click', function() {
    ...
    document.querySelector('.what-bird-description').classList.remove('hidden');
    ...
    document.querySelector('.what-bird__img').innerHTML = `<img src='${targetBird.image}'>`;
    document.querySelector('.what-bird__name').innerHTML = targetBird.name;
    document.querySelector('.what-bird__species').innerHTML = targetBird.species;
...
const birdSongTime = () => {
    if (!audioQur.ended) {
        ...
        let timeQurSong = parseInt((audioQur.currentTime / songLength) * songWidth.clientWidth);
...
const answerBirdSongTime = () => {
    if (!audioAnswer.ended) {
        ...
        let answerTimeQurSong = parseInt((audioAnswer.currentTime / answerSongLength) * answerSongWidth.clientWidth);
...
let answerVolumeNow = document.querySelector('.what-sound__sound-control__value');
...
let gallOn = localStorage.getItem('gallery') ? JSON.parse(localStorage.getItem('gallery')) : false;
isGalleryOn(gallOn);
...
function isGalleryOn() {
    if (gallOn) {
        ...
        document.querySelector('.gallery__link').classList.add('gallery__link-active');
    } else {
        ...
        document.querySelector('.gallery__link').classList.remove('gallery__link-active');
    }
};
